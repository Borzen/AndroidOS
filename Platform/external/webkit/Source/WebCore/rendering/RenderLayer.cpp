...
RenderLayer* RenderLayer::stackingContext() const
{
	...
	// SAMSUNG CHANGE +
    if (layer == NULL) {
	RenderObject* ren = renderer();
	if (ren && ren->document() && ren->document()->ownerElement() && ren->document()->ownerElement()->renderer()) {
	    RenderLayer* parentLayer = ren->document()->ownerElement()->renderer()->enclosingLayer();
	    if (parentLayer) 
	    	layer = parentLayer;
	} 
    }
...
// SAMSUNG CHANGE +
void RenderLayer::computeScrollDimensionsAgain()
{
    RenderBox* box = renderBox();
    ASSERT(box);
